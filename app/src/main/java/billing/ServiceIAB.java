...
import static billing.IabHelper.*;
...
public class ServiceIAB extends IAB {
    ...
    public boolean connect(Context context, final OnConnectListener listener) {
        NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",9");
        // Connection to IAB service
        logger.logDebug("Starting in-app billing setup.");
        mServiceConn = new ServiceConnection() {
            @Override
            public void onServiceDisconnected(ComponentName name) {
                NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",10");
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",11");
                logger.logDebug("Billing service connected.");
                if (disposed()) {
                    NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",12");
                    return;
                }
                NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",13");
                mSetupDone = true;
        ...
        NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",14");
        //        ApplicationClass appClass = (ApplicationClass) context.getApplicationContext();
        Market market = AppModule.INSTANCE.provideMarket();
        NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",15");
        Intent serviceIntent = new Intent(market.getIntentBilling());
        NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",16");
        serviceIntent.setPackage(market.getPackageName());
        NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",17");

        PackageManager pm = context.getPackageManager();
        List<ResolveInfo> intentServices = pm.queryIntentServices(serviceIntent, 0);
        if (intentServices != null && !intentServices.isEmpty()) {
            NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",18");
            // service available to handle that Intent
            return context.bindService(serviceIntent, mServiceConn, Context.BIND_AUTO_CREATE);
        } else {
            NativeBillingV4Helper.Companion.setErrorTrace(NativeBillingV4Helper.Companion.getErrorTrace() + ",19");
            return false;
        }
    }
